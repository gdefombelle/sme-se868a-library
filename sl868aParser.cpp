...
void
Sl868a::sl868a_parse_zda(uint8_t in[], uint8_t in_len)
{
    uint8_t comma_day = 2; // n. ',' before alt data
    uint8_t comma_month = 3;
    uint8_t comma_year = 4;
    uint8_t *ptr;
    sl868aCachedDataT &gpsRxData = smeGps.getChachedDataPtr();
    ...
    ptr = sl868a_parse_param_offset(in, in_len, comma_day);
    while (ptr &&  (*ptr != ',')) {
            gpsRxData.utc_dayOfMonth = (gpsRxData.utc_dayOfMonth *10) + (*ptr -'0');         
        ptr++;
    }
    ptr = sl868a_parse_param_offset(in, in_len, comma_month);
    while (ptr &&  (*ptr != ',')) {
            gpsRxData.utc_month = (gpsRxData.utc_month *10) + (*ptr -'0');         
        ptr++;
    }
    ptr = sl868a_parse_param_offset(in, in_len, comma_year);
    while (ptr &&  (*ptr != ',')) {
            gpsRxData.utc_year = (gpsRxData.utc_year *10) + (*ptr -'0');         
        ptr++;
    }        
